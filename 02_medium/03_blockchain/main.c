...
char *calculate_hash(block_t *block) {
    ...
    sprintf(hash, "%p%p%ld%d", block->data, block->prev_hash, block->timestamp, block->nonce); // aggiunto il nonce all'hash
    return hash;
}

int validate_block(block_t *block, int difficulty) {
    char prefix[difficulty + 1];
    memset(prefix, '0', difficulty);
    prefix[difficulty] = '\0';
    return strncmp(block->hash, prefix, difficulty) == 0;
}
